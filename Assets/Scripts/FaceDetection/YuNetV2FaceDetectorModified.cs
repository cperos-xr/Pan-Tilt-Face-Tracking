...
namespace OpenCVForUnityExample.DnnModel
{
    ...
    /// </summary>
    public class YuNetV2FaceDetectorModified
    {
        Size input_size;
        ...
        public delegate void Vector2EventHandler(Vector2 position);
        public static event Vector2EventHandler OnFaceCenterVector2Sent;

        public delegate void FaceOffsetEventHandler(Vector2 offsetPixels);
        public static event FaceOffsetEventHandler OnFaceOffsetVector2Sent;


        public YuNetV2FaceDetectorModified(string modelFilepath, string configFilepath, Size inputSize, float confThreshold = 0.6f, float nmsThreshold = 0.3f, int topK = 5000, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
        {
            // initialize
        ...
        public virtual void visualize(Mat image, Mat results, bool print_results = false, bool isRGB = false)
        {
            ...
            foreach (var d in data.Reverse())
            {
                ...
                // show a debug statement with the center of the faces coordinates
                Vector2 faceCenter = new Vector2((left + right) / 2, (top + bottom) / 2);
                OnFaceCenterVector2Sent?.Invoke(faceCenter);

                Vector2 frameCenter = new Vector2(image.width() / 2f, image.height() / 2f);
                Vector2 offsetFromCenter = faceCenter - frameCenter;
                Debug.Log($"Face offset from center (pixels): {offsetFromCenter}");

                OnFaceOffsetVector2Sent?.Invoke(offsetFromCenter);
